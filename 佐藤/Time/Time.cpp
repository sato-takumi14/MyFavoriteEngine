...
void Time::Set(float end)
{
	mEndTime = static_cast<float>(duration_cast<seconds>(system_clock::now().time_since_epoch()).count()) + end;
}

const bool Time::GetMeasureTime()
{

	if (mEndTime <= static_cast<float>(duration_cast<seconds>(system_clock::now().time_since_epoch()).count())) {
		mEndTime = 0.0f;
		return true;
	}
...
//const int Time::SetMeasure(DWORD end)
//{
//	mMeasure[mIndex].start = timeGetTime();
//	mMeasure[mIndex].end = mMeasure[mIndex].start + (end * 1000);
//
//	return mIndex++;
//}
//
//const bool Time::GetMeasureTime(const int id)
//{
//	if (mMeasure[id].end <= mNowTime) {
//		mMeasure.erase(id);
//		return true;
//	}
//
